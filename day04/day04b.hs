...
mostCommon = maximum . map (\xs -> (length xs, head xs)) . L.group . L.sort

toMostCommons :: [(Int, [Int])] -> [(Int, Int, Int)]
toMostCommons [] = []
toMostCommons ((g, mins):gs) = let (c, m) = mostCommon mins
                                in (g, c, m) : toMostCommons gs
...
main = do
  ...
  let commons = toMostCommons totalTimes
  let (g, c, m) = L.maximumBy (comparing $ \(g,c,m) -> c) commons
  print $ g * m
